...
<cfargument name="sFileName" type="string" required="false">

<cfset var stDef = this.stDefinitions[ arguments.sSprocName ]>
<cfset var sItem = "">
...
<!--- <cfif NOT structKeyExists( stDef.stArgs.sFileName )>
	<cfset arguments.sFileName = replace( arguments.sSprocName, "gsp_", "qry_", "one" )>
</cfif> --->

<!--- New buffer for sproc --->
<cfset this.addBuffer( "cfm_#arguments.sSprocName#", "cfm", true )>
...
<cfif structKeyExists( stDef.stArgs, "sSetSelect" )>
	<cfset this.append( ' procedure="#stDef.stArgs.sSprocName###arguments.selectSet##"', false )>
<cfelse>
	<cfset this.append( ' procedure="#stDef.stArgs.sSprocName#"', false )>
</cfif>
...
<cfset this.indent()>
	...
	<cfloop index="sItem" list="#stDef.stArgs.lParamFields#">
		...
		<cfset this.append( ' dbVarName="#stDef.stFields[ sItem ].varName#"' )>
		<cfif findNoCase( "OUT", stDef.stFields[ sItem ].type )>
			<cfset this.append( ' variable="attributes.#sItem#"' )>
		</cfif>
		<cfif findNoCase( "IN", stDef.stFields[ sItem ].type )>
			<cfset this.append( ' value="##attributes.#sItem###"' )>
		</cfif>
		<cfset this.appendOn( ' cfSqlType="#stDef.stFields[ sItem ].cfSqlType#"' )>
		<cfif len( stDef.stFields[ sItem ].maxLength )>
			<cfset this.append( ' maxLength="#stDef.stFields[ sItem ].maxLength#"' )>
		</cfif>
		<cfif len( stDef.stFields[ sItem ].scale ) AND listFindNoCase( "decimal,double,float,money,smallmoney", stDef.stFields[ sItem ].sqlType )>
			<cfset this.append( ' scale="#stDef.stFields[ sItem ].scale#"' )>
		</cfif>
		<cfif stDef.stFields[ sItem ].nullable OR len( stDef.stFields[ sItem ].default )>
			<cfset this.append( ' null="##( NOT len( attributes.#sItem# ) ? true : false )##"' )>
		<cfelse>
...
<cfif structKeyExists( stDef.stArgs, "sSetSelect" )>
	<cfset this.append( '<cfset "##attributes.r_statusCode##" = evaluate( "#stDef.stArgs.sSprocName###attributes.selectSet##.statusCode" )>' ) />
<cfelse>
...
<cfset this.append( sLComment & ' Actual Stored Procedure T-SQL Code' & this.sNewLine )>
<cfset this.append( stDef.sSql )>
<cfset this.appendBlank()>
<cfset this.append( sRComment )>

<cfif structKeyExists( stDef.stArgs, "sFilename" )>
	<cfset this.writeBuffer( stDef.stArgs.sFileName, "cfm_#arguments.sSprocName#", "cfm" )>
</cfif>
